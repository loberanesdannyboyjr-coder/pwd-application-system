...
/* ===============================
   QUERY
================================ */
$where = ["a.workflow_status = 'cho_approved'"];
$params = [];
$paramIndex = 1;
...
<form method="get" class="d-flex gap-2 mb-3">
    ...
    <?php if ($search): ?>
        <a href="accepted.php" class="btn btn-outline-secondary">Clear</a>
    <?php endif; ?>
</form>
...
<tr>
    ...
    <th>Action</th>
        ...
        <?php foreach ($rows as $r):
            ...
            $type = strtolower($r['application_type'] ?? '');
        ...
        <td>
            <?php
            if ($type === 'renew') {
                echo '<span class="badge bg-primary">Renewal</span>';
            } elseif ($type === 'lost') {
                echo '<span class="badge bg-warning text-dark">Lost ID</span>';
            } else {
                echo '<span class="badge bg-success">New</span>';
            }
            ?>
            </td>
                    <td><?= !empty($r['application_date'])
...
<?php if ($type === 'renew'): ?>
    ...
    <a href="approve_renew.php?id=<?= urlencode($r['application_id']) ?>" 
...
<?php elseif ($type === 'lost'): ?>
    ...
    <a href="reissue_pwd.php?id=<?= urlencode($r['application_id']) ?>" 
...
<?php else: ?>
    ...
    <a href="issue_pwd.php?id=<?= urlencode($r['application_id']) ?>" 
